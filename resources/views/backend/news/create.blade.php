    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header flex-column flex-md-row">
                <h5 class=" float_left">Add News</h5>
                <div class="dt-action-buttons text-end pt-3 pt-md-0">
                    <div class="dt-buttons"> <a href="{{ route('admin.news.index') }}"> <button
                                class="btn btn-primary" type="button">News List</button> </a> </div>
                </div>
            </div>
            <div class="card-body">
                {{ Form::model($news, ['route' => ['admin.news.store'], 'role' => 'form', 'id'=>'frmNews', 'class' => 'row g-3', 'method'=>'post']) }}
                @if($news->id)
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mt-3">
                        <div class="form-floating form-floating-outline">
                            {!! Form::text('layout', isset($news->category->layout) ? ' Layout '.$news->category->layout : '', ['class' => 'form-control','readonly','id' => 'layout']) !!}
                            <label for="layout">Layout</label>
                        </div>
                    </div>

                    @php
                        $newsType = config('constants.NEWS_TYPE');
                    @endphp
                    <div class="col-md-6 mt-3">
                        <div class="form-floating form-floating-outline">
                            {!! Form::select('news_type', $newsType,null, ['class' => 'form-select  form-select-lg', 'id' => 'news_type', 'placeholder' => 'Select News Type']) !!}
                            <label for="news_type">News Type <span class="text-danger">*</span></label>
                            <span class="text-danger validation-class" id="news_type-error"> </span>
                        </div>
                    </div>

                    @php
                    $viewType = config('constants.VIEW_TYPE');
                    @endphp
                    <div class="col-md-6 mt-3">
                        <div class="form-floating form-floating-outline">
                            {!! Form::select('view_type[]', $viewType,null, ['class' => 'form-select  form-select-lg', 'id' => 'view_type', 'placeholder' => 'Select View Type']) !!}
                            <label for="view_type">View Type <span class="text-danger">*</span></label>
                            <span class="text-danger validation-class" id="view_type-error"> </span>
                        </div>
                    </div>

                    <div class="col-md-6 mt-3">
                        <div class="form-floating form-floating-outline">
                            {!! Form::select('state_id[]', $states,null, ['class' => 'form-select', 'id' => 'state_id', 'placeholder' => 'Select state']) !!}
                            <label for="state_id">State <span class="text-danger">*</span></label>
                            <span class="text-danger validation-class" id="state_id-error"> </span>                            
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
      ...
      $('#category_id').change(function () {
            ...
            $.ajax({
                url: '{{ route('admin.news.layout') }}', // Replace with your actual route
                type: 'POST',
                data: {
